# Azure Managed Database Connectivity

Connect Devin to Azure managed databases — Azure SQL Managed Instance and Azure Database for PostgreSQL Flexible Server — via direct connect, service principal, or customer-hosted private endpoint.

> **Network path is only required when there is no public route to the target resource.** If the database has a public endpoint and you allowlist Devin's static egress IPs, no tunnel or private endpoint is needed. When private networking is required, several options exist: [Azure Bastion Tunneling](../bastion-tunneling/), Zscaler ZPA, [VPN Gateway](../vpn-gateway/), or [Private Endpoints](../private-endpoints/). Choose the option that fits your existing infrastructure.
...
|                                                                      |
|  +---------------------+     +----------------------------------+   |
|  |  App Under Dev       |     |  MCP Database Server              |   |
|  |                      |     |  (org-scoped config)              |   |
|  +----------+-----------+     +--------------+-------------------+   |
|             |                                |                       |
|             +------------+-------------------+                       |
...
+------------------------------+---------------------------------------+
              ...
              |                                                     |
              |  Azure SQL Managed Instance (port 1433)             |
              |  -- or --                                           |
...
| Engine | Azure Service | Port | Client Tool | TLS |
|--------|--------------|------|-------------|-----|
| SQL Server | Azure SQL Managed Instance | 1433 | `sqlcmd` | Always enforced |
| PostgreSQL | Azure Database for PostgreSQL Flexible Server | 5432 | `psql` | Enforced by default (`require_secure_transport = ON`) |

## Architecture Options

Options are ordered from simplest to most infrastructure. Each has sub-variants for both database engines.

| Option | Azure Credential on Devin | Customer Infra | Setup Time | Best For |
|--------|--------------------------|----------------|------------|----------|
| [A: Direct Connect](#option-a-direct-connect) | None -- DB password only | None additional | ~15 min | Simplest possible setup |
...
| [C: Customer-Hosted Private Endpoint](#option-c-customer-hosted-private-endpoint) | None -- DB password only | Private Endpoint + Zscaler/VPN | ~2 hrs | Production / security-sensitive |

---

## Option A: Direct Connect

**Simplest possible setup.** Devin connects directly to the database using a standard connection string over TLS. No Azure identity, no service principal -- just a database password.
...
+--------------------------------------------------------------+--------+
              ...
              |  Customer Azure Subscription                        |
              |                                                     |
              |  Azure SQL MI or PostgreSQL Flexible Server         |
              |  (public endpoint, TLS enforced)                    |
              |  DB user: devin_dev (password auth)                 |
              +----------------------------------------------------+
```
...
- Database **public access must be enabled** (unless routed through Zscaler ZPA to a private endpoint)
- No Entra ID token-based auth -- standard password only
- No short-lived tokens -- DB password should be rotated every 90 days

### A1: Azure SQL Managed Instance

SQL MI is VNet-integrated by default and has no public endpoint unless explicitly enabled. Once the public endpoint is enabled (or Devin reaches the MI via Zscaler ZPA / VPN), Devin connects with standard SQL auth.

See [Direct Connect — SQL MI Setup](docs/option-a1-direct-connect-sql-mi.md) for detailed steps.

**Summary:**
1. Customer enables SQL MI public endpoint (or configures a network path to the MI's VNet)
2. Customer creates a SQL auth login and user (`devin_dev`) with schema-scoped permissions
3. Customer adds Devin's static egress IPs to the MI's NSG rules (or configures Zscaler)
4. Devin stores `DB_HOST`, `DB_USER`, `DB_PASSWORD`, `DB_NAME` as org-scoped secrets

### A2: Azure Database for PostgreSQL Flexible Server

Flexible Server can have a public endpoint with firewall rules. Devin connects with standard PostgreSQL password auth.

See [Direct Connect — PostgreSQL Setup](docs/option-a2-direct-connect-postgresql.md) for detailed steps.
...
**Recommended for POC.** Customer creates an Entra ID service principal, stores its client ID and secret as Devin secrets, and Devin acquires an Entra ID access token at session start for token-based database authentication.
...
+------------------------------+---------------------------------------+
              ...
              |  Entra ID principal mapped to DB role               |
              |                                                     |
...
- The service principal needs no broad Azure AD roles -- it only authenticates to the database via an Entra ID access token

### B1: Azure Database for PostgreSQL Flexible Server

Devin acquires an Entra ID token for the `oss-rdbms` resource type and uses it as the PostgreSQL password via the `pgaadauth` extension.

See [Service Principal — PostgreSQL Setup](docs/option-b1-service-principal-postgresql.md) for detailed steps.

**Summary:**
1. Customer creates an Entra ID app registration and service principal
2. Customer adds the service principal as a PostgreSQL role in the Flexible Server (via `pgaadauth_create_principal`)
...
### B2: Azure SQL Managed Instance

Devin acquires an Entra ID token for the `https://database.windows.net/` resource and uses it for SQL MI token-based authentication via the external user mapped to the service principal.

See [Service Principal — SQL MI Setup](docs/option-b2-service-principal-sql-mi.md) for detailed steps.

**Summary:**
1. Customer creates an Entra ID app registration and service principal
2. Customer creates a contained database user (`CREATE USER [devin-db-sp] FROM EXTERNAL PROVIDER`) in the MI database
3. Devin admin stores `AZURE_TENANT_ID`, `AZURE_CLIENT_ID`, `AZURE_CLIENT_SECRET`, `MI_HOST`, `DB_NAME` as org-scoped secrets
4. Environment blueprint installs Azure CLI in `initialize`, acquires token in `maintenance`

---

## Option C: Customer-Hosted Private Endpoint
...
- **Database public endpoint can remain disabled** -- traffic flows entirely over private link
- **Devin blueprint is minimal** -- no Azure CLI, no token acquisition logic
- **Extends existing network patterns** -- if Zscaler ZPA already routes Devin traffic (e.g., for Azure DevOps), adding the database is an incremental ZPA config change

### C1: Azure SQL Managed Instance

SQL MI is already VNet-integrated, so it inherently has a private endpoint. The customer exposes the MI's VNet-internal address to Devin via Zscaler ZPA or VPN Gateway, and Devin authenticates with SQL auth.

See [Private Endpoint — SQL MI Setup](docs/option-c1-private-endpoint-sql-mi.md) for detailed steps.

**Summary:**
1. Customer configures network access from Devin to the MI's VNet (Zscaler ZPA or VPN Gateway)
2. Customer creates a SQL auth login and user (`devin_dev`) with schema-scoped permissions
3. Devin stores `DB_HOST`, `DB_USER`, `DB_PASSWORD`, `DB_NAME` as org-scoped secrets

### C2: Azure Database for PostgreSQL Flexible Server

Customer creates an Azure Private Endpoint for the Flexible Server and exposes it to Devin via Zscaler ZPA or VPN Gateway.

See [Private Endpoint — PostgreSQL Setup](docs/option-c2-private-endpoint-postgresql.md) for detailed steps.
...
|---|---|---|
| **Static IP Allowlist** | Simplest option; target has a public endpoint | Add [Devin's static egress IPs](https://docs.devin.ai/admin/common-issues#ip-whitelisting) to database firewall / NSG rules |
| **Zscaler ZPA** | Organization already uses Zscaler for Devin traffic (e.g., for Azure DevOps) | Add the database private endpoint or public hostname as a ZPA Application Segment |
...
| `Directory.Reader` | Resolve Entra ID group memberships | Only if the DB role maps to an Entra group |

- The service principal authenticates to the database via an Entra ID access token
- DB-level access is controlled entirely through the database role mapped to the SP
...
| DB password (Options A, C) | Devin org-scoped secret | Every 90 days or on personnel change | Standard SQL / PostgreSQL password |
| DB host / server name | Devin org or repo-scoped secret | N/A (not sensitive, but varies per environment) | Format: `SERVER.postgres.database.azure.com` (PG) or `MI_NAME.HASH.database.windows.net` (SQL MI) |
...
```
cloud-sql/
+-- README.md                                         # This file -- overview + 3 options x 2 engines
+-- docs/
|   +-- option-a1-direct-connect-sql-mi.md            # A1: Direct Connect — SQL MI
|   +-- option-a2-direct-connect-postgresql.md        # A2: Direct Connect — PostgreSQL
|   +-- option-b1-service-principal-postgresql.md     # B1: Service Principal — PostgreSQL
|   +-- option-b2-service-principal-sql-mi.md         # B2: Service Principal — SQL MI
|   +-- option-c1-private-endpoint-sql-mi.md          # C1: Private Endpoint — SQL MI
|   +-- option-c2-private-endpoint-postgresql.md      # C2: Private Endpoint — PostgreSQL
|   +-- wif-future-considerations.md                  # Why federated identity doesn't work today
+-- examples/
    +-- blueprint-direct-connect-postgresql.yaml       # Blueprint for A2
    +-- blueprint-direct-connect-sql-mi.yaml           # Blueprint for A1
    +-- blueprint-service-principal-postgresql.yaml    # Blueprint for B1
    +-- blueprint-service-principal-sql-mi.yaml        # Blueprint for B2
```
...
- [Azure SQL (SQL Server)](../sql/) -- connectivity patterns for Azure SQL Database (single/elastic pool)
...
- [Entra ID Authentication for Azure SQL](https://learn.microsoft.com/en-us/azure/azure-sql/database/authentication-aad-overview)
...
- [Azure Private Link for SQL MI](https://learn.microsoft.com/en-us/azure/azure-sql/managed-instance/private-endpoint-overview)
